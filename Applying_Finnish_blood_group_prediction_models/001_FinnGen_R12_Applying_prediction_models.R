###############################################################################
### Applying blood group imputation models for
### FinnGen R12 genotype data
...
models_info <- readRDS("data/FinnGen_R12_Blood_group_prediction_model.rds")
...
modified_dosage <- map(names(important_variables), function(x) {
  ...
  IMPORTANT_BIM <- left_join(IMPORTANT_VARS, EXT_BIM, 
                             by = c("Model_variant" = "Input_dosage_variant"))
...
# Save in RDS format
saveRDS(Prediction_results, file = "results/FinnGen_R12_Blood_group_prediction_results.rds")
...
fwrite(Prediction_table, "results/FinnGen_R12_Blood_group_predictions_PP",
       sep = " ", col.names = T)
...
fwrite(Antigen_table, "results/FinnGen_R12_BG_prediction_antigen_pos", 
       sep = " ", col.names = T)
